...
GALAXY_INV = {
    "月面基地建設": {"cost": 10000000000000, "share": 500},  # 10兆
    "火星テラフォーミング": {"cost": 50000000000000, "share": 2500}, # 50兆
    "銀河証券取引所設立": {"cost": 100000000000000, "share": 10000} # 100兆
}
...
def run_settlement(months=1):
    ...
    if len(st.session_state.price_history) > 20: st.session_state.price_history = st.session_state.price_history[-20:]
...
if st.session_state.page == "メイン":
    ...
    with t1:
        ...
        for i, n in enumerate([10, 50, 100, 1000]):
            with cols[i % 2]:
                if st.button(f"{n}人採用 ({u_cost*n/100000000:.2f}億)"):
        ...
        for i, (name, cost) in enumerate(FACILITIES.items()):
            with cols[i % 2]:
                if not st.session_state.get(f"f_{name}"):
                    c = int(cost * mult)
                    if st.button(f"{name} ({c/100000000:.0f}億)"):
    ...
    with t3:
        ...
        if st.button(f"1兆円M&A (実費:{int(1e12*mult)/1e12:.1f}兆)"):
            if st.session_state.money >= 1e12*mult: st.session_state.money -= int(1e12*mult); st.session_state.share += 15; st.balloons(); st.rerun()

elif st.session_state.page == "株":
    st.line_chart(st.session_state.price_history)
    s1, s2 = st.columns(2)
    s1.metric("株価", f"{st.session_state.stock_price:,}円")
    s2.metric("保有", f"{st.session_state.stock_owned:,}株")
    if st.button("1万株購入"):
        c = st.session_state.stock_price * 10000
        if st.session_state.money >= c: st.session_state.money -= c; st.session_state.stock_owned += 10000; st.rerun()
